...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        // Check if this post already exists in popularposts
        $check_query = mysqli_query($con, "SELECT id FROM popularposts WHERE id = '$related_post_id'");
        
        if (mysqli_num_rows($check_query) == 0) {
            // Insert into topposts table using direct column mapping
            $insert_query = "INSERT INTO popularposts 
                            SELECT *
                            FROM tblposts 
                            WHERE id = '$related_post_id'";
            
            if (mysqli_query($con, $insert_query)) {
                $_SESSION['msg'] = "Adventure successfully added to popular Adventures!";
            } else {
                $_SESSION['error'] = "Error: " . mysqli_error($con);
            }
        } else {
            $_SESSION['error'] = "This adventure is already in popular Adventures!";
        }
    } else {
